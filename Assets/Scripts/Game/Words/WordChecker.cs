        ...
        for (int x = 0; x < gamePlayManager.mapGenerator.mapSizeX; x++)
            for (int y = 0; y < gamePlayManager.mapGenerator.mapSizeY; y++)
            {
                ...
                if (IsTileRaw(tile)) //«начит что сыра€ клетка
                {
                    ...
                    bool isFistWord = gamePlayManager.numberOfStep == 0 && gamePlayManager.personManager.persons[gamePlayManager.idPlayingPerson].id == gamePlayManager.personManager.persons[PhotonNetwork.LocalPlayer.ActorNumber - 1].id;
